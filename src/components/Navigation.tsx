...
export const Navigation: React.FC<NavigationProps> = ({
  ...
  const navItems = [
    ...
    },
    {
      ...
      icon: "palette",
      tag: "Meals",
      matches: (tab: ActiveTab) =>
        tab === "meals" || tab === "atelier" || tab === "meal-detail",
    },
    {
      ...
      <nav className="hidden md:flex flex-col items-center py-8 bg-[#141313]/95 border-r border-[#444748]/20 fixed left-0 top-0 h-full w-24 z-50 backdrop-blur-xl">
        ...
        >
          <div className="w-11 h-11 rounded-full border border-[#444748]/40 flex items-center justify-center mb-1 group-hover:border-[#c8c6c5] group-hover:scale-105 transition-all duration-300 bg-[#1c1b1b]">
            <span className="font-display-md text-[#e5e2e1] italic text-lg leading-none">
              E
            </span>
          </div>
          <span className="font-label-caps text-[9px] text-[#c4c7c7] tracking-widest uppercase">
            ETHOS
          </span>
        </div>
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              <span
                className={`font-label-caps text-[9px] tracking-wider uppercase mt-1 relative z-10 ${isActive ? "font-medium" : ""}`}
              >
                {item.tag}
              </span>
            </button>
