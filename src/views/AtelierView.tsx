...
export const AtelierView: React.FC<AtelierViewProps> = ({
  ...
  const [mealTitle, setMealTitle] = useState(initialMealToEdit?.title || 'Wild Salmon & Quinoa');
  const [mealType, setMealType] = useState<Meal['type']>(initialMealToEdit?.type || 'Lunch');
  ...
  const totalCals = Math.round(
    ingredients.reduce((sum, ing) => sum + (ing.amountGrams * ing.kcalPer100g) / 100, 0)
  );
  const totalProtein = Math.round(
    ingredients.reduce((sum, ing) => sum + (ing.amountGrams * ing.proteinPer100g) / 100, 0)
  );
  const totalCarbs = Math.round(
    ingredients.reduce((sum, ing) => sum + (ing.amountGrams * ing.carbsPer100g) / 100, 0)
  );
  const totalFat = Math.round(
    ingredients.reduce((sum, ing) => sum + (ing.amountGrams * ing.fatPer100g) / 100, 0)
  );
      ...
      <div className="flex-1 flex flex-col gap-12">
        ...
        <section className="space-y-6">
          ...
          <div className="flex flex-wrap gap-3 items-center pt-2">
            <span className="font-label-caps text-xs text-[#c4c7c7] mr-2">Meal Type:</span>
            {(['Breakfast', 'Lunch', 'Snack', 'Dinner', 'Post-Workout'] as const).map((type) => (
              <button
                key={type}
                type="button"
        ...
        <section className="space-y-4 flex-1">
          <div className="flex justify-between items-end border-b border-[#444748]/20 pb-4 mb-4">
            <div>
              <h2 className="font-headline-sm text-2xl text-[#e5e2e1]">Ingredients</h2>
              <p className="text-[13px] text-[#c4c7c7] mt-0.5">
                Adjust grams directly to calculate macro distribution in real time.
              </p>
            </div>
            <button
          ...
          {ingredients.length === 0 ? (
            ...
            <div className="flex flex-col divide-y divide-[#444748]/20 border-t border-b border-[#444748]/20">
              {ingredients.map((item, index) => {
                const itemCals = Math.round((item.amountGrams * item.kcalPer100g) / 100);
                const itemPro = Math.round((item.amountGrams * item.proteinPer100g) / 100);
                const itemCarbs = Math.round((item.amountGrams * item.carbsPer100g) / 100);
                const itemFat = Math.round((item.amountGrams * item.fatPer100g) / 100);
                    ...
                    <div className="flex items-center gap-3.5 flex-1">
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-[12px] text-[#8e9191] mt-0.5">
                          {itemCals} kcal · {itemPro}g P · {itemCarbs}g C · {itemFat}g F
                        </span>
                      </div>
                    </div>
            ...
            <div className="grid grid-cols-3 gap-2.5">
              <div className="flex flex-col gap-1 bg-[#141313] p-2.5 rounded border border-[#444748]/20">
                ...
                <div className="flex items-baseline gap-0.5">
                  <span className="font-data-highlight text-lg text-[#e5e2e1]">{totalProtein}</span>
              ...
              <div className="flex flex-col gap-1 bg-[#141313] p-2.5 rounded border border-[#444748]/20">
                ...
                <div className="flex items-baseline gap-0.5">
                  <span className="font-data-highlight text-lg text-[#e5e2e1]">{totalCarbs}</span>
                  <span className="text-[11px] text-[#c4c7c7]">g</span>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-1 bg-[#141313] p-2.5 rounded border border-[#444748]/20">
                ...
                <div className="flex items-baseline gap-0.5">
                  <span className="font-data-highlight text-lg text-[#e5e2e1]">{totalFat}</span>
                  <span className="text-[11px] text-[#c4c7c7]">g</span>
                </div>
              </div>
            </div>
          ...
          <div className="fixed top-0 right-0 h-full w-full max-w-md z-[100] bg-[#141313]/95 backdrop-blur-2xl border-l border-[#444748]/30 flex flex-col p-8 shadow-2xl animate-fade-in">
            <div className="flex justify-between items-center border-b border-[#444748]/30 pb-4 mb-6">
              <div className="flex items-center gap-2 flex-1 mr-4">
                <span className="material-symbols-outlined text-[#c4c7c7] text-[20px]">search</span>
                <input
                  autoFocus
                  type="text"
                  value={searchQuery}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="font-label-caps text-[10px] text-[#c4c7c7]">
                        {food.kcalPer100g} kcal / 100g • {food.proteinPer100g}P • {food.carbsPer100g}C • {food.fatPer100g}F
                      </span>
                    </div>
                  </div>
