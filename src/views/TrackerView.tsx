...
export const TrackerView: React.FC<TrackerViewProps> = ({
  ...
  const totalProtein = Math.round(
    allItems.reduce((sum, item) => sum + item.protein, 0),
  );
  const totalCarbs = Math.round(
    allItems.reduce((sum, item) => sum + item.carbs, 0),
  );
  const totalFat = Math.round(
    allItems.reduce((sum, item) => sum + item.fat, 0),
  );
  ...
  const proPercent = Math.min(
    100,
    (totalProtein / userProfile.targetProtein) * 100,
  );
  const carbsPercent = Math.min(
    100,
    (totalCarbs / userProfile.targetCarbs) * 100,
  );
    ...
    <div className="w-full max-w-[1200px] mx-auto px-4 md:px-8 py-8 flex flex-col gap-12 animate-fade-in relative pb-28">
      ...
      >
        ...
        <motion.div
          ...
          className="border border-[#444748]/30 bg-[#191818] p-5 rounded-xl flex flex-col justify-between h-36"
        >
          <div>
          ...
          <div>
            <div className="flex justify-between text-[12px] text-[#8e9191] mb-1.5">
              ...
              <span>
                {Math.max(0, userProfile.targetKcal - totalKcal)} left
              </span>
            </div>
        ...
        >
          <div>
            ...
            <div className="flex items-baseline gap-1.5">
              <span className="font-data-highlight text-3xl text-[#e5e2e1]">
                <NumberTicker value={totalProtein} />g
              </span>
              <span className="text-[13px] text-[#c4c7c7]">
          ...
          <div>
            <div className="flex justify-between text-[12px] text-[#8e9191] mb-1.5">
              ...
              <span>
                {Math.max(0, userProfile.targetProtein - totalProtein)}g left
              </span>
            </div>
            <div className="w-full bg-[#201f1f] h-1 rounded-sm overflow-hidden relative">
              <motion.div
        ...
        >
          <div>
            ...
            <div className="flex items-baseline gap-1.5">
              <span className="font-data-highlight text-3xl text-[#e5e2e1]">
                <NumberTicker value={totalCarbs} />g
              </span>
              <span className="text-[13px] text-[#c4c7c7]">
          ...
          <div>
            <div className="flex justify-between text-[12px] text-[#8e9191] mb-1.5">
              ...
              <span>
                {Math.max(0, userProfile.targetCarbs - totalCarbs)}g left
              </span>
            </div>
            <div className="w-full bg-[#201f1f] h-1 rounded-sm overflow-hidden relative">
              <motion.div
                      ...
                      items.map((item) => (
                        ...
                        >
                          <div className="flex flex-col">
                            ...
                            <span className="font-label-caps text-[10px] text-[#c4c7c7]">
                              {item.amountText} • {item.protein}g P |{" "}
