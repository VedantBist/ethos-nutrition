...
export const FoodDetailView: React.FC<FoodDetailViewProps> = ({
  ...
  const [quantityGrams, setQuantityGrams] = useState<number>(
    food.servingDefaultGrams || 150,
  );
  const [mealDestination, setMealDestination] = useState<
    "breakfast" | "lunch" | "snack" | "dinner"
  >("lunch");
  const [isAdded, setIsAdded] = useState(false);
  const servings =
    food.servings?.filter((serving) => serving.quantityInGrams != null) ?? [];
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8 lg:gap-16 items-start">
        ...
        <motion.div
          ...
          className="md:col-span-5 h-[340px] md:h-[480px] bg-[#1a1a1a] rounded-xl overflow-hidden relative border border-[#444748]/30"
        >
          <img
            src={
              food.image ||
        ...
        >
          <div>
            ...
            <motion.h1
              ...
              className="font-display-lg text-3xl md:text-4xl text-[#e5e2e1] mt-1 mb-2 font-medium"
            >
              {food.name}
            </motion.h1>
            <p className="text-[14px] text-[#c4c7c7] leading-relaxed">
              {food.description ||
                "Nutrient-dense ingredient evaluated per 100g standard baseline with precise macro tracking."}
            </p>
          </div>
          ...
          <div className="p-6 border border-[#444748]/30 rounded-xl bg-[#191818]">
            ...
            <div className="grid grid-cols-4 gap-3">
              <div className="flex flex-col">
                <span className="font-label-caps text-[10px] text-[#8e9191] uppercase">
                  Protein
                </span>
          ...
          <div className="space-y-4 bg-[#191818] p-5 rounded-xl border border-[#444748]/30">
            ...
            <div className="flex items-center justify-between border-b border-[#444748]/20 pb-3">
              <label
                htmlFor="qty"
                className="text-[13px] text-[#c4c7c7] font-medium"
              >
                Quantity (grams)
              </label>
              <div className="flex items-center gap-1.5 border-b border-[#444748]/40 pb-0.5">
                <input
                  ...
                  value={quantityGrams}
                  onChange={(e) =>
                    setQuantityGrams(parseFloat(e.target.value) || 0)
                  }
            ...
            {servings.length > 0 && (
              <div className="flex items-center justify-between border-b border-[#444748]/20 pb-3">
                ...
                <select
                  ...
                  value={
                    servings.find(
                      (serving) => serving.quantityInGrams === quantityGrams,
                    )?.id ?? ""
                  }
                  onChange={(e) => {
                    ...
                    if (serving?.quantityInGrams)
                      setQuantityGrams(serving.quantityInGrams);
                  }}
            ...
            <div className="flex justify-between items-center text-[12px] text-[#8e9191] pt-1">
              ...
              <span className="text-[#bacbbc]">
                {scaledKcal} kcal · {scaledProtein}g P · {scaledCarbs}g C ·{" "}
                {scaledFat}g F
              </span>
            </div>
          </div>
